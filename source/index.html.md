...
data-transcribe | 0 | If set to 1, Cloud Poodll will transcribe the audio in the file to text. This will only work when the region is useast1. No other region supports transcription with Amazon. This process can take several minutes. Eventually notification that the process has finished will come to the notificationurl. But for now a text file containing the transcript will become available of the same filename as the recorded file with an extra ".txt" file extension.
